...
import sqlite3, time, hashlib, feedparser
...
def add_user(user_id, ref_by=None):
    ...
    if cursor.fetchone():
        return  # Fake/ref tekrar engellendi
...
def start(msg):
    ...
    if not check_join(uid):
        ...
        "🚨 Devam etmek için aşağıdaki kanallara katıl:\n\nKatıldıktan sonra butona bas.",
        reply_markup=markup)
        return
...
def callback(call):
    ...
    if call.data.startswith("check_"):
        ...
        if not check_join(uid):
            bot.answer_callback_query(call.id, "❌ Tüm kanallara katıl!")
            return
...
📩 Ödül için: @Weghrumi2""")
    ...
    # ÖDÜL PANEL
        ...
        text = f"👥 Referansın: {refs}\n\n"

        if refs >= 5:
            text += "🎉 5 referans tamamlandı!\n➡️ Turkcell 1GB kazandın\n📩 @Weghrumi2 ile iletişime geç"
        elif refs >= 3:
            text += "🎉 3 referans tamamlandı!\n➡️ Vodafone 1-3GB kazandın\n📩 @Weghrumi2 ile iletişime geç"
        else:
            text += "❌ Henüz ödül yok.\n\n🎯 Hedefler:\n• 3 kişi → Vodafone\n• 5 kişi → Turkcell"
